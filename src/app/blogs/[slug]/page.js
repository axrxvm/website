...
export function generateMetadata({ params }) {
  ...
  const ogImage = image
    ? image
    : `https://itzaarav.netlify.app/api/og?title=${encodeURIComponent(title)}`;
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              image: post.metadata.image
                ? `https://itzaarav.netlify.app${post.metadata.image}`
                : `/og?title=${encodeURIComponent(post.metadata.title)}`,
              url: `https://itzaarav.netlify.app/blogs/${post.slug}`,
